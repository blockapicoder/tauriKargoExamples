import { BufferVector, Features, FeaturesWithBuffer } from "./compute";
export async function compute(features: Features, points: number[][]): Promise<number[][]> {
  ...
  const bufferVector = webgpu.features.createBufferVectorInput(points.length)
  points.forEach((v) => {
  ...
  const result: number[][] = []
  const values: number[] = Array.from(r5)
  for (let j = 0; j < points.length; j++) {
    const tmp: number[] = []
    result.push(tmp)
    for (let i = 0; i < features.dimS; i++) {
      tmp.push(values[j + i * points.length])
    }
  }
  return result
