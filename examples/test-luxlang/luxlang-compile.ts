...
function lowerProgram(namedProg: RawProgram): LoweredStmt[] {
    ...
    // Collecte des var refs (ordre stable) pour capture (RAW)
    ...
    // Collecte var refs sur LOWERED (pour capturer un thunk ifRet)
    function collectVarRefsOrderedLoweredExpr(expr: LoweredExpr, outArr: string[], seen: Set<string>) {
        ...
        if (expr.type === "call") {
            ...
            for (const a of (expr.args ?? [])) {
                if (a.type === "var") {
                    if (!seen.has(a.name)) { seen.add(a.name); outArr.push(a.name); }
                }
            }
            return;
        }
    ...
    function lowerInstr(ctx: Ctx, ins: RawInstr): LoweredInstr[] {
        ...
        if (ins.kind === "ifRet") {
            ...
            if (rt.prelude.length === 0) {
                return [...rc.prelude, { kind: "ifRet", if: rc.expr, then: rt.expr }];
            }

            // Sinon: thunk hoisté => then devient un call(thunk, captures...)
            const thunkLocals = new Set<string>();
            for (const pi of rt.prelude) {
                if (pi.kind === "setLocal") thunkLocals.add(pi.name);
            }

            const used: string[] = [];
            const seen = new Set<string>();
            for (const pi of rt.prelude) collectVarRefsOrderedLoweredInstr(pi, used, seen);
            ...
            return [...rc.prelude, { kind: "ifRet", if: rc.expr, then: thenCall }];
        }
...
function resolveProgram(lowered: LoweredStmt[], knownPrims: Set<string>, globalIndex: Map<string, number>): Prog {
    ...
    function resolveAtom(a: Atom, locals: Map<string, number> | null): Literal | Var {
        ...
        if (a.type === "var") {
            if (knownPrims.has(a.name)) return { type: "extern", name: a.name };
    ...
    function resolveVarOnly(a: RawVarRef, locals: Map<string, number> | null): Cell {
        ...
        if ((r as any).type === "extern") throw new Error("Expected local/global var, got extern: " + JSON.stringify(a));
        return r as Cell;
    }

    function resolveOp(op: RawCallOp, locals: Map<string, number> | null): string | Cell {
        if (typeof op === "string") {
            // si tu veux verrouiller: décommente
    ...
    function resolveExpr(e: LoweredExpr, locals: Map<string, number> | null): Expr {
        ...
        if (e.type === "var") {
            if (knownPrims.has(e.name)) return { type: "extern", name: e.name };
