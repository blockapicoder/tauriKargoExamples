...
import { Call, Expr, IfRet, Literal, Prog, SetVar, Var, Cell } from "./luxlang-model";
...
export type RawStatement = RawFunDef;
...
export interface RawIfBlockStmt {
    kind: "ifBlock";
    if: RawExpr;
    code: RawInstr[];
    ret: RawExpr;
...
type LoweredStmt = LoweredFunDef;
...
export function compileProgram(
    ...
    lowered.forEach((v, i) => {
        globalIndex.set(v.name, i);
    });

    return resolveProgram(lowered, knownPrims, globalIndex);
}

// ============================================================
// 4) Lowering + hoisting + capture (+ ifRet lazy-thunk)
// ============================================================

function v(name: string): RawVarRef {
...
function lowerProgram(namedProg: RawProgram): LoweredStmt[] {
    ...
    function collectVarRefsOrderedExpr(expr: RawExpr, outArr: string[], seen: Set<string>) {
        ...
        if (expr.type === "call") {
            ...
            for (const a of expr.args ?? []) {
                collectVarRefsOrderedExpr(a, outArr, seen);
            }
    ...
    function collectVarRefsOrderedInstr(ins: RawInstr, outArr: string[], seen: Set<string>) {
        ...
        if (ins.kind === "ifBlock") {
            ...
            for (const bi of ins.code) {
                collectVarRefsOrderedInstr(bi, outArr, seen);
            }

            collectVarRefsOrderedExpr(ins.ret, outArr, seen);
            return;
        }
    ...
    function collectVarRefsOrderedLoweredExpr(expr: LoweredExpr, outArr: string[], seen: Set<string>) {
        if (!expr) return;
        ...
        throw new Error("collectVarRefsOrderedLoweredExpr: unknown expr " + JSON.stringify(expr));
    }

    function collectVarRefsOrderedLoweredInstr(ins: LoweredInstr, outArr: string[], seen: Set<string>) {
        if (ins.kind === "setLocal" || ins.kind === "setGlobal") {
            collectVarRefsOrderedLoweredExpr(ins.value, outArr, seen);
            return;
        }

        if (ins.kind === "ifRet") {
            collectVarRefsOrderedLoweredExpr(ins.if, outArr, seen);
            collectVarRefsOrderedLoweredExpr(ins.then, outArr, seen);
            return;
        }
        ...
        throw new Error("collectVarRefsOrderedLoweredInstr: unknown instruction " + JSON.stringify(ins));
    }
    ...
    function lowerExpr(
        ...
        expr: RawExpr,
        wantAtom: boolean
    ): { prelude: LoweredInstr[]; expr: LoweredExpr } {
        ...
        if (expr.type === "lambda" || expr.type === "lambdaBlock") {
            ...
            for (const ins of code) {
                collectVarRefsOrderedInstr(ins, used, seen);
            }
            ...
            if (!wantAtom || isAtom(produced)) {
                return { prelude: [], expr: produced };
            }

            const t = freshTemp();

            return {
                prelude: [mkTempSet(ctx, t, produced)],
                expr: v(t),
            };
        }
        ...
        if (expr.type === "call") {
            ...
            for (const a of expr.args ?? []) {
                ...
                if (!isAtom(r.expr)) {
                    throw new Error("Expected atom after lowering arg");
                }

                atoms.push(r.expr);
            }

            const callFlat: LoweredCallExpr = {
                type: "call",
                op: expr.op,
                args: atoms,
            };
        ...
        throw new Error(
            "Unknown expr node (expected literal/var/call/lambda/lambdaBlock): " +
            JSON.stringify(expr)
        );
    }
    ...
    function lowerCallStmt(ctx: Ctx, st: RawCallStmt): LoweredInstr[] {
        ...
        for (const a of st.args ?? []) {
            ...
            if (!isAtom(r.expr)) {
                throw new Error("Expected atom after lowering call-stmt arg");
            }
    ...
    function lowerIfBlock(ctx: Ctx, ins: RawIfBlockStmt): LoweredInstr[] {
        const rc = lowerExpr(ctx, ins.if, false);

        // Locaux explicites du bloc.
        const blockLocalNames = new Set<string>();

        for (const bi of ins.code) {
            if (bi.kind === "setLocal") {
                blockLocalNames.add(bi.name);
            }
        }

        // Le bloc voit les locaux externes + ses locaux propres.
        const blockLocals = new Set<string>(ctx.locals ?? []);

        for (const n of blockLocalNames) {
            blockLocals.add(n);
        }
        ...
        let blockCode: LoweredInstr[] = [];

        for (const bi of ins.code) {
            blockCode.push(...lowerInstr(blockCtx, bi));
        }

        const rr = lowerExpr(blockCtx, ins.ret, false);
        blockCode.push(...rr.prelude);

        // Locaux propres au thunk, y compris les temporaires générés.
        const thunkLocals = new Set<string>();

        for (const bi of blockCode) {
            if (bi.kind === "setLocal") {
                thunkLocals.add(bi.name);
            }
        }
        ...
        for (const bi of blockCode) {
            collectVarRefsOrderedLoweredInstr(bi, used, seen);
        }

        collectVarRefsOrderedLoweredExpr(rr.expr, used, seen);

        const outerLocals = ctx.locals ? new Set(ctx.locals) : new Set<string>();

        const captures = used.filter(n =>
            outerLocals.has(n) && !thunkLocals.has(n)
        );

        const fname = freshLambda();

        const thunk: LoweredFunDef = {
            kind: "fun",
            name: fname,
            params: captures,
            code: blockCode,
        ...
        hoistedFuns.push(thunk);

        const thenCall: LoweredCallExpr = {
            type: "call",
            op: v(fname),
            args: captures.map(n => v(n)),
        };
    ...
    function lowerInstr(ctx: Ctx, ins: RawInstr): LoweredInstr[] {
        ...
        if (ins.kind === "ifRet") {
            ...
            // Pas de prelude => OK : pas d'effet de bord précoce.
            if (rt.prelude.length === 0) {
                return [
                    ...rc.prelude,
                    {
                        kind: "ifRet",
            ...
            // Sinon : thunk hoisté => then devient call(thunk, captures...).
            const thunkLocals = new Set<string>();

            for (const pi of rt.prelude) {
                if (pi.kind === "setLocal") {
                    thunkLocals.add(pi.name);
                }
            }
            ...
            for (const pi of rt.prelude) {
                collectVarRefsOrderedLoweredInstr(pi, used, seen);
            }

            collectVarRefsOrderedLoweredExpr(rt.expr, used, seen);

            const outerLocals = ctx.locals ? new Set(ctx.locals) : new Set<string>();
            const captures = used.filter(n => outerLocals.has(n) && !thunkLocals.has(n));
    ...
    function lowerFun(st: RawFunDef): LoweredFunDef {
        ...
        // Seuls les setLocal directs de la fonction sont des locaux directs.
        // Les setLocal dans ifBlock restent locaux au thunk généré.
        for (const ins of st.code) {
            if (ins.kind === "setLocal") {
                localNames.add(ins.name);
            }
        }
...
function resolveProgram(
    lowered: LoweredStmt[],
    knownPrims: Set<string>,
    globalIndex: Map<string, number>
): Prog {
    const globalDeclared = new Set<string>(globalIndex.keys());

    let nextG = globalIndex.size;

    function declareGlobal(name: string) {
        globalDeclared.add(name);

        if (!globalIndex.has(name)) {
            globalIndex.set(name, nextG++);
        }
    }
    ...
    function resolveAtom(a: Atom, locals: Map<string, number> | null): Literal | Var {
        ...
        if (a.type === "var") {
            ...
            if (locals && locals.has(a.name)) {
                return {
                    type: "local",
                    idx: locals.get(a.name)!,
                };
            }

            if (globalDeclared.has(a.name)) {
                return {
                    type: "global",
                    idx: ensureGlobal(a.name),
                };
            }
    ...
    function resolveVarOnly(a: RawVarRef, locals: Map<string, number> | null): Cell {
        const r = resolveAtom(a, locals);

        if ((r as any).type === "literal") {
            throw new Error("Expected var, got literal: " + JSON.stringify(a));
        }

        if ((r as any).type === "extern") {
            throw new Error("Expected local/global var, got extern: " + JSON.stringify(a));
        }

        return r as Cell;
    }

    function resolveOp(op: RawCallOp, locals: Map<string, number> | null): string | Cell {
        if (typeof op === "string") {
            // Si tu veux verrouiller :
            // if (!knownPrims.has(op)) throw new Error(`Unknown primitive op '${op}'`);
            return op;
        }
        ...
        if (globalDeclared.has(name)) {
            return {
                type: "global",
                idx: ensureGlobal(name),
            };
        }

        if (knownPrims.has(name)) {
            return name;
        }
    ...
    function resolveExpr(e: LoweredExpr, locals: Map<string, number> | null): Expr {
        if (e.type === "literal") {
            return e;
        }

        if (e.type === "var") {
            if (knownPrims.has(e.name)) {
                return {
                    type: "extern",
                    name: e.name,
                };
            ...
            if (locals && locals.has(e.name)) {
                return {
                    type: "local",
                    idx: locals.get(e.name)!,
                };
            }

            if (globalDeclared.has(e.name)) {
                return {
                    type: "global",
                    idx: ensureGlobal(e.name),
                };
            }
